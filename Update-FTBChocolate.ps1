...
if (Test-Path "$env:LOCALAPPDATA\.ftba\Instances") {
    ...
    foreach ($Folder in $Folders) {
        if ((Get-Content -Path "$env:LOCALAPPDATA\.ftba\Instances\$($Folder.Name)\instance.json") -like "*FTB Chocolate*") {
            $script:InstancePath = "$env:LOCALAPPDATA\.ftba\Instances\$($Folder.Name)"
    ...
    $script:InstancePath = Read-Host "Instance Folder Path"
}
...
Rename-Item -Path "$script:InstancePath\Mods\create-mc1.18.2_v0.4.1.jar" -NewName "$script:InstancePath\Mods\create-mc1.18.2_v0.4.1.disabled"
Invoke-WebRequest -Uri "https://github.com/chazza6464/Modpacks/blob/main/Mod%20Files/create-1.18.2-0.5.0.e.jar" -OutFile "$script:InstancePath\Mods\create-1.18.2-0.5.0.e.jar"
Write-Host "(1/6) Downloaded Create" -ForegroundColor Yellow

# Flywheel
# Disable 0.6.2, Install 0.6.5
Rename-Item -Path "$script:InstancePath\Mods\flywheel-forge-1.18-0.6.2.jar" -NewName "$script:InstancePath\Mods\flywheel-forge-1.18-0.6.2.jar.disabled"
Invoke-WebRequest -Uri "https://github.com/chazza6464/Modpacks/blob/main/Mod%20Files/flywheel-forge-1.18.2-0.6.5.jar" -OutFile "$script:InstancePath\Mods\flywheel-forge-1.18.2-0.6.5.jar"
Write-Host "(2/6) Downloaded Flywheel" -ForegroundColor Yellow

# Chipped
# Install 2.0.0
Invoke-WebRequest -Uri "https://github.com/chazza6464/Modpacks/raw/main/Mod%20Files/chipped-2.0.0-forge.jar" -OutFile "$script:InstancePath\Mods\chipped-2.0.0-forge.jar"
Write-Host "(3/6) Downloaded Chipped" -ForegroundColor Yellow

# OpenBlocks Elevator
# Install 1.8.4
Invoke-WebRequest -Uri "https://github.com/chazza6464/Modpacks/blob/main/Mod%20Files/elevatorid-1.18.2-1.8.4.jar" -OutFile "$script:InstancePath\Mods\elevatorid-1.18.2-1.8.4.jar"
Write-Host "(4/6) Downloaded OpenBlocks Elevator" -ForegroundColor Yellow

# Iron Chests
# Install 13.2.11
Invoke-WebRequest -Uri "https://github.com/chazza6464/Modpacks/blob/main/Mod%20Files/ironchest-1.18.2-13.2.11.jar" -OutFile "$script:InstancePath\Mods\ironchest-1.18.2-13.2.11.jar"
Write-Host "(5/6) Downloaded Iron Chests" -ForegroundColor Yellow

# FTB Ultimine
# Install 1802.3.3
Invoke-WebRequest -Uri "https://github.com/chazza6464/Modpacks/blob/main/Mod%20Files/ftb-ultimine-forge-1802.3.3-build.67.jar" -OutFile "$script:InstancePath\Mods\ftb-ultimine-forge-1802.3.3-build.67.jar"
